...
class VariationalAutoencoder(object):
    ...
    def _sample_z(self, z_mean, z_log_var):
        ...
        eps = tf.random_normal(tf.shape(z_log_var))

        # z = mu + sigma*epsilon
        #z_log_var stands for log (sigma^2)
    ...
    def _encoder(self, x):
        ...
        representing z_mean and z_log_var.
        ...
        l1 = tf.contrib.layers.fully_connected(x,100,tf.nn.softplus)
        l2 = tf.contrib.layers.fully_connected(l1,50,tf.nn.softplus)
        
        z_mean = tf.contrib.layers.fully_connected(l2,self._nlatent,activation_fn=None)
        z_log_var = tf.contrib.layers.fully_connected(l2,self._nlatent,activation_fn=None)
    ...
    def _decoder(self, z):
        ...
        l1 = tf.contrib.layers.fully_connected(z,50,tf.nn.softplus)
        l2 = tf.contrib.layers.fully_connected(l1,100,tf.nn.softplus)
        #need to add activation_fn=None so that f can be negative
        f = tf.contrib.layers.fully_connected(l2,self._ndims,activation_fn=None)
    ...
    def _latent_loss(self, z_mean, z_log_var):
        ...
        D_KL = -0.5*(1 + z_log_var - tf.square(z_mean) - tf.exp(z_log_var))
        latent_loss = tf.reduce_mean(D_KL,1)
    ...
    def _reconstruction_loss(self, f, x_gt):
        ...
        """
        #use l2_loss square
        recon_loss = tf.reduce_sum((0.5*tf.square((x_gt-f))),1)
    ...
    def loss(self, f, x_gt, z_mean, z_var):
        ...
        """
        total_loss = tf.reduce_mean(self._reconstruction_loss(f,x_gt) + self._latent_loss(z_mean,z_var))
